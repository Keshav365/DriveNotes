...
const NoteModal: React.FC<NoteModalProps> = ({
  ...
  // AI Functions - Work directly on content for instant preview
  const callAIDirectly = async (aiFunction: string, instructions = '') => {
    if (!content.trim()) {
  ...
  const handleAISummarize = async () => {
    const result = await callAIDirectly('summarize');
    if (result) {
  ...
  const handleAICreateTable = async () => {
    const result = await callAIDirectly('create-table');
    if (result) {
      setContent(prev => `${prev}\n\n## Generated Table\n\n${result.table}`);
    }
  };
  ...
  const handleAIEnhance = async () => {
    const result = await callAIDirectly('enhance');
    if (result) {
  ...
  const handleAIExpand = async () => {
    const result = await callAIDirectly('expand');
    if (result) {
  ...
  const handleAIFormat = async () => {
    const result = await callAIDirectly('format');
    if (result) {
      ...
      >
        <motion.div
          ...
          className={`
            ...
            overflow-hidden border-2 ${colorConfig.border} flex flex-col
          `}
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-6 space-y-6">
              ...
              {/* AI Tools - Available for both creation and editing */}
              <div className="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 p-4 rounded-lg border border-purple-200 dark:border-purple-800/50">
                <div className="flex items-center justify-between mb-3">
                  ...
                  >
                    ...
                    <span>{showAITools ? 'Hide Tools' : 'Show AI Tools'}</span>
                  </button>
                </div>
                ...
                {!content.trim() && (
                  <div className="text-xs text-purple-600 dark:text-purple-400 mb-2 flex items-center space-x-1">
                    <span>💡</span>
                    <span>Add some content to unlock AI-powered features like summarization, enhancement, and more!</span>
                  </div>
                )}
                  ...
                  {showAITools && (
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      <button
                        ...
                        className="p-3 text-sm bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 text-purple-700 dark:text-purple-300 rounded-lg hover:from-purple-100 hover:to-purple-200 dark:hover:from-purple-800/30 dark:hover:to-purple-700/30 transition-all duration-200 flex flex-col items-center space-y-1 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {aiProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <FileText className="w-4 h-4" />}
                        <span>Summarize</span>
                      </button>
                      ...
                      <button
                        ...
                        disabled={aiProcessing}
                        className="p-3 text-sm bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 text-green-700 dark:text-green-300 rounded-lg hover:from-green-100 hover:to-green-200 dark:hover:from-green-800/30 dark:hover:to-green-700/30 transition-all duration-200 flex flex-col items-center space-y-1 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {aiProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Table className="w-4 h-4" />}
                      ...
                      <button
                        ...
                        disabled={aiProcessing}
                        className="p-3 text-sm bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 text-blue-700 dark:text-blue-300 rounded-lg hover:from-blue-100 hover:to-blue-200 dark:hover:from-blue-800/30 dark:hover:to-blue-700/30 transition-all duration-200 flex flex-col items-center space-y-1 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                      ...
                      <button
                        ...
                        disabled={aiProcessing}
                        className="p-3 text-sm bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 text-orange-700 dark:text-orange-300 rounded-lg hover:from-orange-100 hover:to-orange-200 dark:hover:from-orange-800/30 dark:hover:to-orange-700/30 transition-all duration-200 flex flex-col items-center space-y-1 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {aiProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <ListTodo className="w-4 h-4" />}
                      ...
                      <button
                        ...
                        disabled={aiProcessing}
                        className="p-3 text-sm bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-900/20 dark:to-indigo-800/20 text-indigo-700 dark:text-indigo-300 rounded-lg hover:from-indigo-100 hover:to-indigo-200 dark:hover:from-indigo-800/30 dark:hover:to-indigo-700/30 transition-all duration-200 flex flex-col items-center space-y-1 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {aiProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Expand className="w-4 h-4" />}
                        <span>Expand</span>
                      </button>
                      ...
                      <button
                        ...
                        disabled={aiProcessing}
                        className="p-3 text-sm bg-gradient-to-br from-teal-50 to-teal-100 dark:from-teal-900/20 dark:to-teal-800/20 text-teal-700 dark:text-teal-300 rounded-lg hover:from-teal-100 hover:to-teal-200 dark:hover:from-teal-800/30 dark:hover:to-teal-700/30 transition-all duration-200 flex flex-col items-center space-y-1 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {aiProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <AlignLeft className="w-4 h-4" />}
                        <span>Format</span>
                      </button>
                    </div>
                  )}
