...
export default function SettingsPage() {
  ...
  const [showAiApiKeys, setShowAiApiKeys] = useState({
    openai: false,
    gemini: false,
    claude: false
  });
  ...
  const loadAvailableProviders = async () => {
    try {
      const response = await userAPI.getAvailableAIProviders();
      if (response.data.success) {
        // Backend returns { success: true, data: { providers: [...] } }
        setAvailableProviders(response.data.data.providers || getDefaultAIProviders());
      } else {
        // Fallback to default providers if API fails
        setAvailableProviders(getDefaultAIProviders());
      }
    } catch (error) {
  ...
  // Default AI providers with their models
  const getDefaultAIProviders = () => [
    {
      id: 'openai',
      name: 'OpenAI',
  ...
  const handleAiSettingsSave = async () => {
    try {
      ...
      setMessage({ type: 'error', content: 'Failed to update AI settings' });
    } finally {
      setAiLoading(false);
    }
  };

  const handleSetAiApiKey = async (provider: string, apiKey: string) => {
    if (!apiKey.trim()) {
      setMessage({ type: 'error', content: 'Please enter an API key' });
      return;
    }

    try {
      setAiLoading(true);
      const response = await userAPI.setAIApiKey(provider, apiKey.trim());
      
      if (response.data.success) {
        setMessage({ type: 'success', content: `${provider.toUpperCase()} API key saved successfully!` });
        // Clear the API key field
        setAiApiKeys(prev => ({ ...prev, [provider]: '' }));
        // Reload AI settings to get updated hasApiKeys status
        loadAiSettings();
      }
    } catch (error) {
      setMessage({ type: 'error', content: 'Invalid API key or service unavailable' });
    } finally {
      setAiLoading(false);
    }
  };

  const handleRemoveAiApiKey = async (provider: string) => {
    ...
    try {
      setAiLoading(true);
      const response = await userAPI.removeAIApiKey(provider);
      
      if (response.data.success) {
        setMessage({ type: 'success', content: `${provider.toUpperCase()} API key removed successfully!` });
        // Reload AI settings to get updated hasApiKeys status
        loadAiSettings();
      }
    } catch (error) {
      setMessage({ type: 'error', content: 'Failed to remove API key' });
    } finally {
      setAiLoading(false);
    }
  };
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden transition-colors">
          ...
          <div className="p-6">
            ...
            {activeTab === 'ai' && (
              ...
              >
                <div className="flex items-center justify-between">
                  <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">AI Assistant Settings</h2>
                  <div className="flex items-center space-x-2">
                    <div className="flex items-center space-x-2 text-purple-600 dark:text-purple-400">
                      <Brain className="w-4 h-4" />
                      <span className="text-sm font-medium">Powered by Your API Keys</span>
                    </div>
                  </div>
                </div>

                {/* Provider Selection */}
                <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg border border-purple-200 dark:border-purple-800/50 transition-colors">
                  <h3 className="text-sm font-medium text-purple-900 dark:text-purple-300 mb-3 flex items-center space-x-2">
                      ...
                      <button
                        ...
                        onClick={() => setAiSettings({ 
                          ...
                          preferences: {
                            ...
                            defaultModel: provider.models[0] // Set the first model as default
                          }
                        })}
                        className={`p-4 rounded-lg border-2 transition-all ${
                          aiSettings.provider === provider.id
                            ? 'border-purple-500 bg-purple-100 dark:bg-purple-900/30 text-purple-900 dark:text-purple-300'
                            : 'border-gray-200 dark:border-slate-600 hover:border-purple-300 dark:hover:border-purple-500 text-gray-700 dark:text-gray-300'
                        }`}
                      >
                ...
                <div className="space-y-4">
                  {availableProviders.map((provider) => (
                    <div key={provider.id} className="bg-gray-50 dark:bg-slate-700 p-4 rounded-lg transition-colors">
...
disabled={aiLoading || !aiApiKeys[provider.id as keyof typeof aiApiKeys].trim()}
                        ...
                        {provider.id === 'openai' && (
                          <div className="text-xs text-gray-600 dark:text-gray-300 space-y-1">
                            <p>• Go to <a href="https://platform.openai.com/api-keys" target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:underline inline-flex items-center space-x-1">
                              <span>OpenAI Platform</span>
                              <ExternalLink className="w-3 h-3" />
                            </a></p>
                            <p>• Create a new API key and paste it here</p>
                        ...
                        {provider.id === 'gemini' && (
                          <div className="text-xs text-gray-600 dark:text-gray-300 space-y-1">
                            <p>• Go to <a href="https://makersuite.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:underline inline-flex items-center space-x-1">
                              <span>Google AI Studio</span>
                              <ExternalLink className="w-3 h-3" />
                            </a></p>
                            <p>• Create a new API key and paste it here</p>
                            <p>• Gemini API has generous free tier limits</p>
                          </div>
                        )}
                        
                        {provider.id === 'claude' && (
                          <div className="text-xs text-gray-600 dark:text-gray-300 space-y-1">
                            <p>• Go to <a href="https://console.anthropic.com/" target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:underline inline-flex items-center space-x-1">
                              <span>Anthropic Console</span>
                              <ExternalLink className="w-3 h-3" />
                            </a></p>
                            <p>• Create a new API key and paste it here</p>
                            <p>• Make sure you have credits in your Anthropic account</p>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div className="bg-gray-50 dark:bg-slate-700 p-4 rounded-lg transition-colors">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <select
                        value={aiSettings.preferences.defaultModel}
...
{availableProviders.find((p: { id: string; models: string[] }) => p.id === aiSettings.provider)?.models.map((model: string) => (
                    ...
                    <div>
                      ...
                      <input
                        ...
                        min="100"
                        max="4000"
                        value={aiSettings.preferences.maxTokens}
                        onChange={(e) => setAiSettings({
                          ...aiSettings,
                          preferences: {
                            ...aiSettings.preferences,
                        ...
                        className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors"
                      />
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Maximum tokens per AI response</p>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        max="2"
                        step="0.1"
                        value={aiSettings.preferences.temperature}
                        onChange={(e) => setAiSettings({
                          ...aiSettings,
                          preferences: {
                            ...aiSettings.preferences,
                        ...
                        className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors"
                      />
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">0 = focused, 1 = balanced, 2 = creative</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800/50 transition-colors">
                  <h3 className="text-sm font-medium text-blue-900 dark:text-blue-300 mb-3 flex items-center space-x-2">
                    <Sparkles className="w-4 h-4" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    <div className="flex items-start space-x-2">
                      ...
                      <div>
                        <p className="text-sm font-medium text-blue-900 dark:text-blue-300">Smart Summarization</p>
                        <p className="text-xs text-blue-700 dark:text-blue-400">Automatically summarize long notes</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
                      <div>
                        <p className="text-sm font-medium text-blue-900 dark:text-blue-300">Table Generation</p>
                        <p className="text-xs text-blue-700 dark:text-blue-400">Convert text into structured tables</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
                      <div>
                        <p className="text-sm font-medium text-blue-900 dark:text-blue-300">Content Enhancement</p>
                        <p className="text-xs text-blue-700 dark:text-blue-400">Improve writing and fix grammar</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
                      <div>
                        <p className="text-sm font-medium text-blue-900 dark:text-blue-300">Action Items</p>
                        <p className="text-xs text-blue-700 dark:text-blue-400">Extract tasks and to-dos</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
                      <div>
                        <p className="text-sm font-medium text-blue-900 dark:text-blue-300">Content Expansion</p>
                        <p className="text-xs text-blue-700 dark:text-blue-400">Elaborate on ideas and concepts</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
                      <div>
                        <p className="text-sm font-medium text-blue-900 dark:text-blue-300">Smart Formatting</p>
                        <p className="text-xs text-blue-700 dark:text-blue-400">Organize content with headings</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-4">
                  <button
                    ...
                    disabled={aiLoading}
                    className="flex-1 flex items-center justify-center space-x-2 px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Save className="w-4 h-4" />
